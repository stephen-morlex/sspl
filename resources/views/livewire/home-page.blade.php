                    ...
                    <div class="card-body p-0">
                        ...
                        <div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach ($liveFixtures as $fixture)
                                <article
                                    class="rounded-lg border border-base-content/10 p-4 hover:bg-base-200 transition-colors">
                                    <div
                                        class="flex items-center justify-between text-[13px] text-base-content/70 mb-2">
                                        <span>{{ $fixture->league->name }}</span>
                                        <span
                                            class="inline-flex items-center px-2 h-5 rounded-full text-xs bg-error/10 text-error">LIVE</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-3">
                                            <div class="avatar placeholder">
                                                <div class="mask mask-squircle h-12 w-12">
                                                    <img src="https://ssfa-services.com/images/teams/salam_fc.png"
                                        ...
                                        <div class="text-center">
                                            <div class=" font-semibold badge badge-lg">{{ $fixture->home_score }} -
                                                {{ $fixture->away_score }}</div>
                                            <div class="text-[12px] text-base-content/70 mt-1">
                                                {{ $fixture->kickoff_time->format('H:i') }}</div>
                                        </div>
                                        <div class="flex items-center gap-3">
                        ...
                        @forelse($upcomingFixtures as $fixture)
                            <article
                                ...
                                <div class="mt-2 text-[13px] text-base-content/70 text-center">
                                    {{ $fixture->kickoff_time->format('g:i A') }} at {{ $fixture->venue }}</div>
                            </article>
                        @empty
                            <div class="text-center py-10 text-base-content/70 col-span-2">No upcoming matches
                                scheduled.</div>
                        @endforelse
                    </div>
                </div>
            ...
            <section class="card card-bordered bg-base-100 shadow-sm">
                <div class="card-body p-0">
                                ...
                                @forelse($topStandings as $standing)
                                    <tr class="{{ $standing->position <= 4 ? 'bg-success/10' : 'bg-base-100' }}">
                                        <td class="px-4 py-3 text-sm font-medium">
                                            <span
                                                class="{{ $standing->position <= 4 ? 'text-success' : 'text-base-content' }}">{{ $standing->position }}</span>
                                        </td>
                                        <td class="px-4 py-3">
                                        ...
                                        <td class="px-4 py-3 text-center text-sm text-base-content/70">
                                            {{ $standing->played }}</td>
                                        <td
                                            class="px-4 py-3 text-center text-sm font-semibold {{ $standing->position <= 4 ? 'text-success' : '' }}">
                                            {{ $standing->points }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-4 py-6 text-center text-sm text-base-content/70">No
                    ...
                    <div class="px-4 py-3 bg-base-200 border-t border-base-content/10 text-center text-sm">
                        <a href="#" class="text-primary hover:text-primary-focus font-medium">View Full Standings
                            →</a>
                    </div>
                </div>
            </section>
            ...
            <section class="card card-bordered bg-base-100 shadow-sm">
                <div class="card-body p-0">
                    ...
                    <div class="p-4 grid grid-cols-2 gap-3">
                        <a href="#"
                            class="flex flex-col items-center justify-center p-4 bg-base-200 rounded-lg hover:bg-base-300 transition-colors">
                ...
                <div class="card-body p-0">
                    <div class="px-4 py-3 border-b border-base-content/10">
                        <div class="flex items-center justify-between">
                            ...
                            <a href="#" class="text-[13px] text-base-content/70 hover:underline">View all
                                matches</a>
                        </div>
                    </div>
                        ...
                        @foreach ($upcomingFixtures->take(5) as $f)
                            <li class="px-4 py-3">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="text-sm text-base-content/70">{{ $f->kickoff_time->format('H:i') }}
                                    </div>
                                </div>
                            </li>
                        @endforeach
